...
import { Button, DateInput, Spinner } from '@components/atoms';
import { SearchBar, FilterDropdown, EmptyState, Pagination } from '@components/molecules';
import { formatDate } from '@utils/helpers';

const LIMIT = 20;

const buildQuery = ({ search, tipoId, empresaId, fechaDesde, fechaHasta, page }) => {
  const q = { estado: 'pendiente_asignacion', page, limit: LIMIT };
...
const BandejaAsignacion = () => {
  ...
  const { data, isLoading } = useGetTramitesQuery(
    buildQuery({ search, tipoId, empresaId, fechaDesde, fechaHasta, page }),
  );
  ...
  const hasFilters = search || tipoId || empresaId || fechaDesde || fechaHasta;
  ...
  const emptyDesc  = hasFilters
    ...
    : 'No hay trámites esperando asignación en este momento.';
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-display-sm text-primary font-semibold">Asignación de Trámites</h1>
        <p className="text-body-md text-on-surface-variant mt-1">
          Trámites pendientes de asignación a oficinas revisoras
        </p>
      </div>
